    ...
    <AdminLayout>
        <div class="space-y-8 animate-fade-in">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <Link :href="admin.roles.create ? admin.roles.create.url() : '#'" class="bg-brand-600 hover:bg-brand-700 text-white px-4 py-2 rounded-xl font-bold text-sm shadow-xl shadow-brand-600/20 flex items-center transition-all active:scale-95">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <!-- Role Card -->
                <div v-for="role in roles" :key="role.id" class="group bg-white dark:bg-slate-800 rounded-xl border border-slate-200/60 dark:border-slate-700 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 p-5 flex flex-col justify-between">
                    <div>
                        <div class="flex justify-between items-start mb-6">
                            <div class="w-12 h-12 rounded-xl flex items-center justify-center shadow-inner transition-transform group-hover:scale-110" :class="getRoleColor(role.name)">
                ...
                <Link :href="admin.roles.create ? admin.roles.create.url() : '#'" class="rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-700 p-5 flex flex-col items-center justify-center hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-all group min-h-[300px]">
