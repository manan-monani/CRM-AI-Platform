...
import { login as customerLogin } from '@/routes';
...
const appName = computed(() => businessSettings.value.business_name || page.props.name || 'Laravel');
...
const demoCredentials = [
    { label: 'Super Admin', email: 'admin@admin.com', password: '12345678', role: 'Super Admin' },
    { label: 'Sales Manager', email: 'manager@admin.com', password: '12345678', role: 'Manager' },
    { label: 'Sales Rep', email: 'employee@crm.com', password: '12345678', role: 'Employee' },
];
    ...
    <div class="min-h-screen flex flex-col lg:flex-row bg-white dark:bg-gray-900 font-sans text-slate-700 dark:text-slate-300 transition-colors duration-300">
        ...
        <div class="lg:w-1/2 branded-bg relative flex flex-col items-center justify-center p-12 text-white overflow-hidden">
            ...
            <div class="relative z-10 max-w-lg text-center lg:text-left">
                <Link href="/" class="flex items-center space-x-3 mb-8 group hover:opacity-90 transition-opacity">
                    <div class="inline-flex items-center justify-center w-10 h-10 bg-white/10 backdrop-blur-md rounded-xl text-white border border-white/20 overflow-hidden shadow-lg transition-transform group-hover:scale-105">
                        <template v-if="logoUrl">
                            <img :src="logoUrl" class="dark:hidden w-full h-full object-contain p-1" />
                            <img :src="logoDarkUrl || logoUrl" class="hidden dark:block w-full h-full object-contain p-1" />
                        </template>
                    ...
                    <span class="text-xl font-extrabold tracking-tight text-white uppercase">{{ appName }}<span class="text-primary-400">.</span></span>
                </Link>
                <h1 class="text-4xl lg:text-6xl font-bold mb-6 leading-tight">{{ businessSettings.admin_login_headline || appName }}</h1>
                <p class="text-primary-100 text-lg lg:text-xl mb-12 font-light leading-relaxed">
                    {{ businessSettings.admin_login_subheadline || businessSettings.tagline }}
                </p>
        ...
        <div class="lg:w-1/2 flex items-center justify-center p-6 md:p-8 bg-white dark:bg-gray-900 transition-colors duration-300 relative">
            ...
            <div class="w-full max-w-md">
                ...
                <div class="lg:hidden text-center mb-10">
                    <Link href="/" class="inline-flex items-center justify-center w-12 h-12 bg-primary-600 dark:bg-primary-900/30 rounded-xl mb-4 text-white p-2">
                        <template v-if="logoUrl">
                            <img :src="logoUrl" class="dark:hidden w-full h-full object-contain" />
                            <img :src="logoDarkUrl || logoUrl" class="hidden dark:block w-full h-full object-contain" />
                        </template>
                        <svg v-else class="w-6 h-6" fill="currentColor" viewBox="0 0 320 512"><path d="M296 160H180.6l42.6-129.8C227.2 15 215.7 0 200 0H56C44 0 33.8 8.9 32.2 20.8l-32 240C-1.7 275.2 25.2 288 40 288h115.4l-42.6 129.8C108.8 434.9 120.3 448 136 448h144c12 0 22.2-8.9 23.8-20.8l32-240c1.9-14.3-9.2-27.2-24-27.2zM250.7 220.3l-32 240c-.6 4.3-4.1 7.7-8.7 7.7h-144c-4.1 0-7.7-2.9-8.4-7L100.2 332H40c-4.1 0-7.7-2.9-8.4-7L-11.4 69.8c-.8-5.8 3.7-10.9 9.5-10.9h144c4.1 0 7.7 2.9 8.4 7L167.8 196H228c4.1 0 7.7 2.9 8.4 7l24.3 15.3z"/></svg>
                    </Link>
                    <h2 class="text-2xl font-bold text-slate-800 dark:text-white">{{ businessSettings.admin_login_welcome_back || __('Welcome Back') }}</h2>
                </div>

                <!-- Header -->
                <div class="mb-10 hidden lg:block">
                    <h2 class="text-2xl font-bold text-slate-800 dark:text-white mb-2">{{ businessSettings.admin_login_form_title || __('Admin Access') }}</h2>
                    <p class="text-slate-500 dark:text-slate-400">{{ businessSettings.admin_login_form_subtitle || __('Sign in to manage your platform') }}</p>
                </div>

                 <div v-if="status" class="mb-6 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 text-green-600 dark:text-green-400 rounded-xl p-4 text-sm font-medium">
                    {{ status }}
                </div>
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <button
                        ...
                        class="w-full py-3 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-xl shadow-xl shadow-primary-200 dark:shadow-none transform transition-all active:scale-95 flex items-center justify-center space-x-2 disabled:opacity-75 disabled:cursor-not-allowed"
                    >
                ...
                <!-- Demo Credentials -->
                <div v-if="$page.props.app_mode === 'demo'" class="mt-8 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
                    <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11.536 9.636a1.003 1.003 0 00-.454-.242l-2.616-.523a1 1 0 00-1.164.908l-.071.712a1 1 0 01-.84.91l-1.498.25-1.498.25a1 1 0 01-1.165-.91l-.071-.712a1 1 0 01.84-.91l2.616-.523a1 1 0 00.454-.242l1.735-3.036a6 6 0 017.743-5.743z"></path></svg>
                        Demo Accounts
                    </h3>
                    <div class="grid grid-cols-1 gap-2">
                        <button 
                            ...
                            type="button"
                            class="flex items-center justify-between p-2 hover:bg-white dark:hover:bg-slate-700 rounded-lg transition-colors group text-left border border-transparent hover:border-slate-200 dark:hover:border-slate-600"
                        >
                            <div class="flex flex-col">
                                <span class="text-sm font-bold text-slate-700 dark:text-slate-200">{{ cred.label }}</span>
                                <span class="text-xs text-slate-500 dark:text-slate-400">{{ cred.email }}</span>
                            </div>
                            <span class="text-xs font-medium text-primary-600 dark:text-primary-400 opacity-0 group-hover:opacity-100 transition-opacity bg-primary-50 dark:bg-primary-900/30 px-2 py-1 rounded">
                                Auto Fill
                            </span>
                        </button>
                    </div>
