...
onMounted(() => {
    ...
    // Theme Init - Default to light unless explicitly dark
    if (localStorage.theme === 'dark') {
        isDark.value = true;
        // Ensure class is present (sync with app.blade.php)
...
const confirmLogout = () => {
    isUserDropdownOpen.value = false; // Close dropdown before showing modal
...
<template>
    <header class="h-16 flex items-center justify-between px-4 lg:px-6 bg-admin-header dark:bg-admin-header-dark backdrop-blur-md border-b border-admin-sidebar-border dark:border-admin-sidebar-border-dark sticky top-0 z-30 transition-colors duration-300">
        <div class="flex items-center gap-4">
            ...
            <button @click="emit('toggleSidebar')" class="lg:hidden w-9 h-9 flex items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all font-sans">
            ...
            <button @click="emit('toggleSidebarSize')" class="hidden lg:flex w-9 h-9 items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:text-sky-600 transition-all font-sans">
                <svg class="w-5 h-5 rtl:scale-x-[-1]" fill="currentColor" viewBox="0 0 448 512"><path d="M0 96C0 78.3 14.3 64 32 64H416c17.7 0 32 14.3 32 32s-14.3 32-32 32H32C14.3 128 0 113.7 0 96zM0 256c0-17.7 14.3-32 32-32H416c17.7 0 32 14.3 32 32s-14.3 32-32 32H32c-17.7 0-32-14.3-32-32zM448 416c0 17.7-14.3 32-32 32H32c-17.7 0-32-14.3-32-32s14.3-32 32-32H416c17.7 0 32 14.3 32 32z"/></svg>
            </button>
            ...
            <div class="text-start font-sans">
                ...
                <p class="text-[10px] text-slate-400 font-extrabold uppercase tracking-[0.2em] text-nowrap mt-0.5">{{ __(t('subtitle')) }}</p>
            </div>
        </div>

        <div class="flex items-center gap-2 md:gap-4 relative font-sans">
            <NotificationBell
            ...
            <button 
                ...
                class="hidden md:flex bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-3 py-1.5 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all items-center shadow-sm disabled:opacity-50 disabled:cursor-not-allowed font-sans" 
                :title="__('Refresh')"
            >
                 <svg 
                    class="w-3 h-3 md:me-2 rtl:scale-x-[-1]" 
                ...
                <span class="hidden sm:inline">{{ isReloading ? __('Refreshing...') : __(t('refresh')) }}</span>
            </button>

            
            <div class="relative inline-block text-start ms-2" ref="dropdownRef">
                <button @click.stop="toggleUserDropdown" class="rounded-full flex items-center justify-center transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-500">
                    <UserAvatar 
                ...
                <!-- Dropdown -->
                <div v-show="isUserDropdownOpen" class="absolute end-0 mt-3 w-56 bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-100 dark:border-slate-800 py-2 z-50 dropdown-animate origin-top-right font-sans">
                    <div class="px-4 py-3 border-b border-slate-50 dark:border-slate-800 text-start">
                        <p class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">{{ __(t('signed_in_as')) }}</p>
                        <p class="text-sm font-extrabold text-slate-900 dark:text-white truncate">{{ $page.props.auth.user.name }}</p>
                    </div>
                    ...
                    <div class="p-1 border-b border-slate-50 dark:border-slate-800 md:hidden">
                        ...
                        <button @click="toggleTheme" class="w-full flex items-center space-x-3 rtl:space-x-reverse px-3 py-2 text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-sky-600 dark:hover:text-sky-400 rounded-xl transition-all font-bold text-start">
                    ...
                    <div class="p-1">
                        <Link :href="customer.profile.edit.url()" class="w-full flex items-center space-x-3 rtl:space-x-reverse px-3 py-2 text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-sky-600 dark:hover:text-sky-400 rounded-xl transition-all font-bold text-start">
                        ...
                        <button @click="confirmLogout" class="w-full flex items-center space-x-3 rtl:space-x-reverse px-3 py-2 text-sm text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/30 rounded-xl transition-all font-bold text-start mt-1">
        ...
        <!-- Logout Confirmation Modal (teleported to body to escape header z-index) -->
        <Teleport to="body">
            <ConfirmationModal
                :show="showLogoutConfirmation"
                :title="__('Log Out')"
                :message="__('Are you sure you want to log out of your account?')"
                :confirm-text="__('Log Out')"
