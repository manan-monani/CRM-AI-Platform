...
class BusinessSettingService
{
    ...
    public function getSettings(array $keys = [], bool $formatForFrontend = false)
    {
        ...
        if ($formatForFrontend) {
            return $settings->map(function ($value, $key) {
                if (in_array($key, ['logo_url', 'favicon_url', 'cover_url', 'logo_dark_url', 'favicon_dark_url']) && $value && !str_starts_with($value, 'http')) {
    ...
    public function updateBranding(array $data, array $files = []): void
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (in_array($key, ['logo_url', 'favicon_url', 'cover_url', 'logo_dark_url', 'favicon_dark_url']) && is_string($value) && str_starts_with($value, '/storage/')) {
